...
export default function ArticleShare({ title, url }: { title: string; url: string }) {
  ...
  function openShare(channel: "facebook" | "telegram") {
    track("education_article_shared", { channel, article: url });
    const destination = channel === "facebook"
      ? `https://www.facebook.com/sharer/sharer.php?display=popup&u=${encodedUrl}&quote=${encodedTitle}`
  ...
  return (
    <aside className="articleShare" aria-label="Կիսվել նյութով">
      ...
      <nav>
        <button type="button" className="shareLogoButton facebookShareButton" onClick={() => openShare("facebook")} aria-label="Կիսվել Facebook-ում" title="Facebook">
        ...
        <button type="button" className="shareLogoButton telegramShareButton" onClick={() => openShare("telegram")} aria-label="Կիսվել Telegram-ում" title="Telegram">
